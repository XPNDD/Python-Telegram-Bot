...
@bot.callback_query_handler(func=lambda call: call.data == 'high_command', state=UserState.chat_started)
def high_callback(call):
    high_cmd(call)


@bot.message_handler(commands=['high'], state=UserState.chat_started)
def high_cmd(message: Message):
    ...
    bot.set_state(message.from_user.id, UserState.high_category_picking)


@bot.message_handler(state=UserState.high_category_picking)
def category_pick(message: Message):
    if message.text in categories_lst:
        ...
        bot.set_state(message.from_user.id, UserState.high_amount_picking)
    else:
...
@bot.message_handler(state=UserState.high_amount_picking)
def pick_amount(message: Message):
    if message.text.isdigit():
        ...
        temp_items_lst = sorted(items_lst, key=lambda dct: dct['price'], reverse=True)
